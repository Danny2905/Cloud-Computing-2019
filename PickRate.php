...
<html lang="en">
  <head>
    ...
    <title>Champion Pick Rate</title>
  </head>
  <body>
...
<nav class="navbar navbar-expand-sm bg-dark navbar-dark">
    ...
    <ul class="navbar-nav">
      <li class="nav-item">
        <a class="nav-link" href="WinRate.php">Win Rate</a>
      </li>
      <li class="nav-item">
        <a class="nav-link" href="BanRate.php">Ban Rate</a>
      </li>
      <li class="nav-item active">
        <a class="nav-link" href="PickRate.php">Pick Rate</a>
      </li>
    </ul>
  </nav>
        ...
        var data = new google.visualization.arrayToDataTable([
          ...
          <?php
            foreach( $result as $row ) {
              echo "['".$row["ChampionName"]."', ".$row["PickRate"]."],"; 
            } ?>
          ]);
        // Set chart options
        var options = {'title':'Champion Pick Rate',
                       width: 1600,
                       height :750, 
                       hAxis : { 
                        textStyle : {
...
<div class = "container">
  ...
  <div class="" id="chart_div"></div>
    ...
    <?php
      ...
      foreach( $result as $row ) {
          fwrite($handle,$row['ChampionName'].','.$row['PickRate'].',');
      }
      fclose($handle);
    ?>
